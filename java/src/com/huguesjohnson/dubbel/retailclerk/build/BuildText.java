...
public class BuildText extends BaseBuilder{
	...
	public static void build(String basePath,TextParameters parameters) throws Exception{
		...
		try{
			//setup table writer
			String tableWriterPath=PathResolver.getAbsolutePath(basePath,parameters.tableFilePath);
			FileUtils.mkdirs(tableWriterPath);
			tableFileWriter=new FileWriter(tableWriterPath);
			tableFileWriter.write("; generated by build tools");
			tableFileWriter.write(newLine);
			//setup text writer
			String textWriterPath=PathResolver.getAbsolutePath(basePath,parameters.textFilePath);
			FileUtils.mkdirs(textWriterPath);
			textFileWriter=new FileWriter(PathResolver.getAbsolutePath(basePath,parameters.textFilePath));
			...
			for(int fileIndex=0;fileIndex<parameters.filePaths.length;fileIndex++){
				String textJson=Files.readString(Paths.get(PathResolver.getAbsolutePath(basePath,parameters.filePaths[fileIndex])));
