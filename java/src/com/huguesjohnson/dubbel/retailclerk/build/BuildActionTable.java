...
public class BuildActionTable extends BaseBuilder{
	...
	public static void build(String basePath,ActionTableParameters actionTable,HashMap<Integer,String> sceneIDMap) throws Exception{
		...
		try{
			...
			for(int actionId=0;actionId<actionTable.actions.length;actionId++){
				actionIdMap.put(actionTable.actions[actionId],Integer.valueOf(actionId));
			}
			String writerPath=PathResolver.getAbsolutePath(basePath,actionTable.filePath);
			FileUtils.mkdirs(writerPath);
			...
			if((actionTable.entries!=null)&&(actionTable.entries.length>0)){
				...
				for(int day=0;day<actionTable.dayCount;day++){
					...
					for(int scene=0;scene<actionTable.sceneCount;scene++){
						Integer sceneId=Integer.valueOf(scene);
						writer.write("; scene ");
						writer.write(String.valueOf(scene));
						writer.write(newLine);
						for(int action=0;action<actionTable.actions.length;action++){
							writer.write("\tdc.l\t");
